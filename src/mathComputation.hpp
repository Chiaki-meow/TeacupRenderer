...
public:
    ...
    // 圆c和直线l的交点
    static pair<Point, Point> getCrossPoints(Circle c, Line l) {
    ...
    static Vector polar(double a, double r) { return Point(cos(r) * a, sin(r) * a); }
    ...
    /*
        ...
        OUT 0
    */
    static int contains(Polygon g, Point p) {
        int n = g.size();
        bool x = false;
        for (int i = 0; i < n; i++) {
    ...
    static int cmp(Point A, Point B)                     //竖直排序  
    {
        return (A.y<B.y || (A.y == B.y&&A.x<B.x));
    }
    // 凸包
    static Polygon andrewScan(Polygon s) {
        Polygon u, l;
        int len = s.size();
        if (len < 3) return s;
